...
export interface VerifyFlowOptions {
  /** When true, follow `crosslinks.superseded_by` instead of halting at superseded atoms. */
...
export function verifyFlow(
  ...
  while (queue.length > 0) {
    ...
    if (entry.status === 'superseded') {
      if (opts.throughSuperseded) {
        const successors = entry.crosslinks?.['superseded_by']
        if (Array.isArray(successors) && successors.length > 0) {
          for (const successor of successors) {
            if (typeof successor !== 'string' || successor.length === 0) continue
            if (visited.has(successor)) {
              errors.push({ id, kind: 'not_approved', reason: `supersede cycle detected: ${id} → ${successor}` })
              continue
            }
            edges.push({ from: id, to: successor, via: 'references' })
      ...
      errors.push({
        id,
        kind: 'not_approved',
        reason: `status is 'superseded'${opts.throughSuperseded ? '; no superseded_by to follow' : ' (use --through-superseded to follow the chain)'}`,
      })
      // fall through to continue walking the rest of the graph
    } else if (!isApprovedStatus(entry.status)) {
      errors.push({
        id,
        kind: 'not_approved',
