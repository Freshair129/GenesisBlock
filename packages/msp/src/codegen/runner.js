...
const defaultMock = async ({ prompt }) => {
...
const defaultAcceptance = async (_task, _code) => {
    // Default — assume pass. Real runners should plug in a vitest invocation.
    return [];
};
async function loadBlueprint(root, blueprintId) {
...
async function loadPackageDeps(root) {
    try {
...
export async function runTask(taskPath, opts = {}) {
    ...
    if (blueprint.status !== 'stable') {
        throw new CodegenError(`parent blueprint ${task.parent_blueprint} is '${blueprint.status}' — must be 'stable' before running tasks`);
    }
    ...
    const slmClient = opts.slmClient ?? createSlmClient({});
