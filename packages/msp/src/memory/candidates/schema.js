...
import { CandidateIdError, } from './types.js';
const ID_PATTERN = /^(CONCEPT|ADR|FEAT|BLUEPRINT|FRAME|AUDIT|PROTO)(?:-[a-zA-Z0-9-]+)?--[A-Z0-9-]+(?:--K\d+)?$/;
export function assertValidProposedId(id) {
    if (!ID_PATTERN.test(id))
        throw new CandidateIdError(id);
}
...
export function composeFrontmatter(input, proposedAt, proposedBy) {
    const lines = ['---'];
    ...
    if (input.rationale !== undefined)
        lines.push(`rationale: ${yamlScalar(input.rationale)}`);
...
export function parseFrontmatter(raw) {
    ...
    if (!match)
        throw new Error('Candidate file: missing or malformed frontmatter');
