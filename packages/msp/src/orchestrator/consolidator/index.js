...
import { deterministicSummary, extractDeterministicTags, } from './summarise.js';
import { DEFAULT_LLM_TIMEOUT_MS, DEFAULT_MAX_LLM_CALLS, DEFAULT_THRESHOLDS, } from './config.js';
...
function sessionPath(root, namespace, sessionId) {
    return resolve(root, '.brain/msp/projects', namespace, 'sessions', `${sessionId}.jsonl`);
}
/**
 * Read a session.jsonl file into an in-memory `Turn[]`. Missing file → [].
 */
export async function readSessionTurns(root, namespace, sessionId) {
