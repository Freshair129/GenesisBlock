...
import { DEFAULT_LLM_TIMEOUT_MS, RESUMMARISE_RATIO, TRIM_DROP_FRACTION, } from './types.js';
export { DEFAULT_LLM_TIMEOUT_MS, RESUMMARISE_RATIO, TRIM_DROP_FRACTION, TRIM_THRESHOLD, } from './types.js';
export { DEFAULT_TOKENISER, estimateText } from './tokens.js';
export { trimEpisode, tier1ScoreSingleTurn } from './trim.js';
export { buildResummarisePrompt, cleanLlmText, resummariseLLM, truncate, } from './resummarise.js';
export { joinTurns } from './text.js';
function buildEpisodeRef(episode) {
    if (episode.atomId) {
        return {
...
function refSortKey(ref) {
    if (ref.turnRange)
        return [ref.sessionId ?? '', ref.turnRange[0]];
...
export async function compress(opts) {
    const tokeniser = opts.tokeniser ?? DEFAULT_TOKENISER;
    ...
    for (const { e: episode } of sorted) {
        ...
        const droppedFraction = episode.turns.length > 0
        ...
        const target = Math.max(1, Math.min(Math.floor(fullTokens * RESUMMARISE_RATIO), remaining));
        if (opts.llm) {
            try {
