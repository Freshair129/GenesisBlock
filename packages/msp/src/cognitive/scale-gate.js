...
export async function enforceScaleGate(input) {
    ...
    for (const t of required) {
        const present = closure.some((a) => (a.type ?? '').toLowerCase() === t && STABLE_STATUSES.has((a.status ?? '').toLowerCase()));
        if (!present)
            missing.push(t.toUpperCase());
    }
    if (missing.length > 0) {
        throw new ScaleLevelGateError(input.scale, missing);
    }
}
async function loadAtoms(root) {
...
async function scanGksDir(gksDir) {
    const out = [];
    let types = [];
    try {
...
function parseFrontmatter(text) {
    ...
    for (const line of m[1].split('\n')) {
        ...
        if (key === 'id')
            ...
            out.status = val;
        else if (key === 'crosslinks' && val.startsWith('{'))
            out.crosslinksJson = val;
    }
    return out;
}
...
export async function predicate(ctx) {
    const violations = [];
    const blueprints = ctx.atomicIndex.filter((a) => a.type === 'blueprint' && (a.status === 'active' || a.status === 'stable'));
