...
export const description = 'k-hop BFS over the Symbol Graph from a given symbol id (depth capped at 3). Returns the center symbol plus reached nodes and traversed edges. Read-only.';
        ...
        try {
            ...
            if (args.edge_types && args.edge_types.length > 0) {
                ...
                for (const t of args.edge_types) {
                    if (KNOWN_EDGE_TYPES.includes(t))
                        valid.push(t);
                }
                edgeTypes = valid;
            }
            ...
            return errorResult(`symbol_neighbors failed: ${err.message}`);
        }
        finally {
            try {
                store.close();
