...
export const description = 'Run MSP retrieval: fan out across GKS vector, Obsidian text, episodic memory, and backlinks; fuse via RRF; return ranked hits with provenance + per-source timings. Read-only.';
export const inputSchema = {
    ...
    weights: z
        ...
        .optional()
        .describe('Per-source RRF weight overrides keyed by source name (gks-vector, obsidian-text, episodic, backlinks).'),
    timeout_ms: z
        .number()
        .int()
        .positive()
 ...
 */
async function tryCreateObsidianClient(opts) {
    try {
