...
export const description = 'Trace execution flow (downward to callees or upward to callers) from a given symbol. Returns multiple paths with depth and cycle info. Read-only.';
...
export function handler(ctx) {
    return async (args) => {
        ...
        try {
            ...
            return errorResult(`symbol_trace failed: ${err.message}`);
        }
        finally {
            try {
                store.close();
