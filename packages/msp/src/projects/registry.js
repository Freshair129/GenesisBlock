...
import { REGISTRY_SCHEMA_VERSION, defaultRegistry, } from './types.js';
/**
...
export async function readRegistry() {
    ...
    try {
        ...
        throw new Error(`projects.yaml at ${path} is not valid YAML: ${err.message}`);
    }
    ...
    if (typeof sv !== 'number') {
        throw new Error(`projects.yaml at ${path} is missing required \`schemaVersion\` (expected 1)`);
    }
    if (sv > REGISTRY_SCHEMA_VERSION) {
        throw new Error(`projects.yaml at ${path} has schemaVersion=${sv}; this build only supports up to ${REGISTRY_SCHEMA_VERSION}`);
    }
    ...
    if (projectsRaw !== undefined) {
        ...
        for (const [name, entry] of Object.entries(projectsRaw)) {
            ...
            if (typeof e.path !== 'string' || e.path.length === 0) {
                throw new Error(`projects.yaml at ${path}: project \`${name}\` is missing required \`path\` field`);
            }
            const cleaned = { path: e.path };
            if (typeof e.embedder === 'string')
                cleaned.embedder = e.embedder;
            if (typeof e.description === 'string')
                cleaned.description = e.description;
 ...
 */
export async function registerProject(name, entry) {
    if (name.length === 0) {
