 ...
 */
async function findMspconfig(start) {
 ...
 */
export async function parseMspconfig(path) {
...
function lookup(registry, name, source) {
    ...
    if (!entry) {
        ...
        throw new Error(`project '${name}' is not registered (resolved via ${source}). Known projects: ${knownStr}. ` +
