 ...
 */
export async function recordEpisode(task, result, root) {
    ...
    const tags = ['agents', 'dispatch', result.tier_used.toLowerCase()];
    const escalatedLine = result.escalated_from
        ? `escalated_from: ${result.escalated_from}\n`
        : '';
    const costLine = typeof result.cost_usd === 'number' ? `cost_usd: ${result.cost_usd}\n` : '';
    const frontmatter = [
        '---',
        `id: ${id}`,
        'phase: 5',
