...
fn sq8_finds_exact_match() {
    ...
    s.create_collection("sq8c".into(), "m".into(), 4, Some("Cosine".into()), Some("sq8".into())).unwrap();
    seed(&s, "sq8c");
...
fn none_finds_exact_match() {
    ...
    s.create_collection("f32c".into(), "m".into(), 4, Some("Cosine".into()), None).unwrap();
    seed(&s, "f32c");
...
fn sq8_disk_is_quarter_of_f32() {
    ...
    let f32_bytes = fs::metadata(Path::new(&path).join("vec_f32c.bin")).unwrap().len();
    let sq8_bytes = fs::metadata(Path::new(&path).join("vec_sq8c.bin")).unwrap().len();
    assert_eq!(f32_bytes, 5 * 4 * 4, "f32: 5 vectors x dim 4 x 4 bytes");
    assert_eq!(sq8_bytes, 5 * 4, "sq8: 5 vectors x dim 4 x 1 byte");
    assert_eq!(f32_bytes, sq8_bytes * 4, "SQ8 arena is exactly 4x smaller on disk");
}
