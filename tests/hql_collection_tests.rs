...
fn search_in_quoted_collection_scopes() {
    ...
    let v = s
        .execute_hql("SEARCH q SIMILAR TO [1.0,0.0,0.0,0.0] K 10 IN \"code\"")
        .unwrap();
    let hits = ids(&v);
    assert!(
        hits.contains(&"code-A".to_string()),
        "code search returns its own vector"
    );
    assert!(
        !hits.contains(&"def-A".to_string()),
        "default vector must not appear in code search"
    );
}
...
fn search_without_in_uses_default() {
    ...
    let v = s
        .execute_hql("SEARCH q SIMILAR TO [1.0,0.0,0.0,0.0] K 10")
        .unwrap();
    let hits = ids(&v);
    assert!(
        hits.contains(&"def-A".to_string()),
        "default search returns the default vector"
    );
    assert!(
        !hits.contains(&"code-A".to_string()),
        "code vector must not appear in default search"
    );
}
...
fn match_in_collection_scopes() {
    ...
    let v = s
        .execute_hql("MATCH q SIMILAR TO [1.0,0.0,0.0,0.0] ALPHA 0.0 IN \"code\"")
        .unwrap();
    let hits = ids(&v);
    assert!(hits.contains(&"code-A".to_string()));
    assert!(!hits.contains(&"def-A".to_string()));
