...
fn top1(s: &Storage, q: Vec<f64>) -> Option<String> {
    ...
    s.hybrid_search(HybridSearchInput {
        query_vector: q, k: 1, alpha: Some(0.0), lang: None, as_of: None, collection: None, ef_search: None,
...
fn legacy_string_meta_migrates() {
    ...
    let legacy = vec![LegacyMeta {
        arena_id: 0, node_id: "L1".to_string(), timestamp: 0, vector_dim: 4,
        embedding_offset: 0, gks_attributes: vec![], lang: "en".to_string(), cluster_id: 0,
    }];
    ...
    let mut state: serde_json::Value = serde_json::from_str(&fs::read_to_string(&sp).unwrap()).unwrap();
    if let Some(colls) = state["collections"].as_array_mut() {
        for c in colls.iter_mut() {
            if c["name"] == "default" { c.as_object_mut().unwrap().remove("mv"); }
        }
    }
    ...
    let s2 = open(&path);
    assert_eq!(top1(&s2, vec![1.0, 0.0, 0.0, 0.0]), Some("L1".to_string()),
    ...
    let restate: serde_json::Value = serde_json::from_str(&fs::read_to_string(&sp).unwrap()).unwrap();
    let dflt = restate["collections"].as_array().unwrap().iter()
        .find(|c| c["name"] == "default").unwrap();
    assert_eq!(dflt["mv"].as_u64(), Some(1), "re-save writes the new meta format");
}
