...
use genesis_block_native::{Storage, OpenOptions, NodeInput, EdgeInput, HybridSearchInput};
use std::fs;
use std::path::Path;
...
fn open(path: &str) -> Storage {
    Storage::open(OpenOptions {
        path: path.to_string(), page_cache_mb: Some(32), read_only: Some(false), vector_dim: Some(4),
    }).unwrap()
}
...
fn add_node(s: &Storage, id: &str) {
    s.add_node(NodeInput {
        ...
        embedding: Some(vec![1.0, 0.0, 0.0, 0.0]), lang: Some("en".to_string()),
...
fn add_edge(s: &Storage, id: &str, from: &str, to: &str) {
    s.add_edge(EdgeInput {
        ...
        props: None, valid_from: Some("2024-02-01T00:00:00Z".to_string()),
        supersede: None, impact: None, caused_by: None,
    }).unwrap();
}

fn wal_len(path: &str) -> u64 {
    fs::metadata(Path::new(path).join("genesis-graph.wal")).map(|m| m.len()).unwrap_or(0)
}
...
fn save_state_shrinks_wal_through_open_handle() {
    ...
    // 100 live nodes ...
    for i in 0..100 { add_node(&s, &format!("N{i}")); }
    ...
    for i in 0..400 { add_edge(&s, &format!("E{i}"), &format!("N{}", i % 100), &format!("N{}", (i + 1) % 100)); }
    for i in 0..400 { s.retract_edge(format!("E{i}"), Some("2025-01-01T00:00:00Z".to_string())).unwrap(); }
    ...
    assert!(after < before, "WAL must shrink after checkpoint (dead history dropped): {before} -> {after}");
    ...
    for i in 400..800 { add_edge(&s, &format!("E{i}"), &format!("N{}", i % 100), &format!("N{}", (i + 1) % 100)); }
    for i in 400..800 { s.retract_edge(format!("E{i}"), Some("2025-01-01T00:00:00Z".to_string())).unwrap(); }
    s.save_state().unwrap();
    let after2 = wal_len(&path);
    assert!(after2 <= after * 2, "WAL stays bounded across repeated checkpoints: {after} then {after2}");
}
...
fn checkpointed_wal_replays_to_identical_live_state() {
    ...
    {
        let s = open(&path);
        for i in 0..50 { add_node(&s, &format!("N{i}")); }
        add_edge(&s, "keep", "N0", "N1");
        add_edge(&s, "drop", "N1", "N2");
        s.retract_edge("drop".to_string(), Some("2025-01-01T00:00:00Z".to_string())).unwrap();
        s.flush_index();
    ...
    let s2 = open(&path);
    s2.flush_index();
    for i in 0..50 { assert!(s2.get_u32(&format!("N{i}")).is_some(), "N{i} survived checkpoint + WAL replay"); }
    assert_eq!(s2.nodes.len(), 50, "exactly the 50 live nodes are reconstructed from the WAL");

    let hits: Vec<String> = s2.hybrid_search(HybridSearchInput {
    ...
    assert!(!hits.is_empty(), "embeddings survived checkpoint + WAL replay and stay searchable");
}
