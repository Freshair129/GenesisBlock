...
fn search(s: &Storage, q: Vec<f64>, k: u32, collection: Option<&str>) -> Result<Vec<String>, String> {
    ...
    s.hybrid_search(HybridSearchInput {
        ...
        collection: collection.map(|c| c.to_string()),
    }).map(|v| v.into_iter().map(|n| n.node.id).collect()).map_err(|e| e.to_string())
}
...
fn two_collections_are_isolated() {
    ...
    s.create_collection("code".to_string(), "jina-code".to_string(), 4, Some("L2".to_string())).unwrap();
    s.create_collection("text".to_string(), "bge-m3".to_string(), 3, Some("Cosine".to_string())).unwrap();
    ...
    assert!(!code_hits.contains(&"text-A".to_string()), "text vector must not appear in code search");

    let text_hits = search(&s, vec![1.0, 0.0, 0.0], 10, Some("text")).unwrap();
    assert_eq!(text_hits, vec!["text-A".to_string()], "text search returns only its own space");
}
...
fn dim_mismatch_is_rejected() {
    ...
    s.create_collection("code".to_string(), "m".to_string(), 4, None).unwrap();
...
fn collections_survive_snapshot_reload() {
    let path = fresh("test_mc_snapshot");
    {
        ...
        s.create_collection("code".to_string(), "jina-code".to_string(), 4, Some("Cosine".to_string())).unwrap();
        add(&s, "c1", vec![1.0, 0.0, 0.0, 0.0], Some("code")).unwrap();
        s.save_state().unwrap();
    }
    let s2 = open_dim(&path, 4);
    let info = s2.list_collections().into_iter().find(|c| c.name == "code").expect("code collection survives");
    ...
    assert_eq!(hits, vec!["c1".to_string()], "vector searchable after reload");
}
...
fn wal_replay_recovers_custom_collection() {
    ...
    {
        let s = open_dim(&path, 4);
        s.create_collection("code".to_string(), "jina-code".to_string(), 4, None).unwrap();
    ...
    let info = s2.list_collections().into_iter().find(|c| c.name == "code").expect("code recovered on replay");
    assert_eq!(info.dim, 4);
