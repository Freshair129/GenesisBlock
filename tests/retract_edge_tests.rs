...
fn retract_hides_from_current_view() {
    ...
    let retracted = s
        .retract_edge("E1".to_string(), Some("2025-01-01T00:00:00Z".to_string()))
        .unwrap();
    assert!(retracted.is_some(), "retract returns the edge");
    assert_eq!(
        retracted.unwrap().valid_to,
        Some("2025-01-01T00:00:00Z".to_string())
    );

    assert!(
        neighbor_ids(&s, "A", None, None).is_empty(),
        "retracted edge hidden from current view"
    );
}
...
fn retracted_visible_as_of_before_retraction() {
    ...
    seed(&s);
    s.retract_edge("E1".to_string(), Some("2025-01-01T00:00:00Z".to_string()))
        .unwrap();

    // Before the retraction (2024-06): edge still live.
    assert_eq!(
        neighbor_ids(&s, "A", Some("2024-06-01T00:00:00Z"), None),
        vec!["B".to_string()]
    );
...
fn include_invalid_surfaces_retracted() {
    ...
    assert_eq!(
        neighbor_ids(&s, "A", None, Some(true)),
        vec!["B".to_string()],
        "shown with include_invalid"
    );
}
...
fn retract_missing_returns_none() {
    ...
    seed(&s);
    assert!(s
        .retract_edge("does-not-exist".to_string(), None)
        .unwrap()
        .is_none());
}
...
fn retraction_persists_across_reopen() {
    ...
    let s2 = open(&path);
    assert!(
        neighbor_ids(&s2, "A", None, None).is_empty(),
        "retraction durable across reopen"
    );
    assert_eq!(
        neighbor_ids(&s2, "A", None, Some(true)),
        vec!["B".to_string()]
    );
}
