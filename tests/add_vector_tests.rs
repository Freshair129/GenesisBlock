...
fn node_carries_vectors_in_two_collections() {
    ...
    s.create_collection("code".to_string(), "jina-code".to_string(), 3, Some("L2".to_string())).unwrap();
...
fn add_vector_to_missing_node_errors() {
    ...
    s.create_collection("code".to_string(), "m".to_string(), 3, None).unwrap();
...
fn attached_vector_survives_wal_replay() {
    let path = fresh("test_av_wal");
    {
        ...
        s.create_collection("code".to_string(), "jina-code".to_string(), 3, None).unwrap();
        add_node(&s, "N1", vec![1.0, 0.0, 0.0, 0.0], None);
