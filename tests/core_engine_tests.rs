use genesis_block_native::{Storage, OpenOptions, NodeInput, EdgeInput, QueryInput, HybridSearchInput};
use std::fs;
use std::path::Path;
...
fn test_edge_creation_and_graph_traversal() {
    ...
    storage.add_node(NodeInput { id: Some("A".to_string()), labels: vec![], props: None, embedding: None }).unwrap();
    storage.add_node(NodeInput { id: Some("B".to_string()), labels: vec![], props: None, embedding: None }).unwrap();
...
fn test_wal_group_commit_durability() {
    ...
    // Phase 1: Write
    {
        ...
        storage.add_node(NodeInput { id: Some("durable_node".to_string()), labels: vec![], props: None, embedding: None }).unwrap();
    } 
