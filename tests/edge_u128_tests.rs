...
fn u128_edges_traverse_and_survive_reload() {
    ...
    assert_eq!(
        hop1_out(&s2, "A"),
        2,
        "u128-keyed edges survive snapshot reload"
    );
}
...
fn legacy_u64_keyed_snapshot_loads() {
    ...
    let mut v: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(&edges_path).unwrap()).unwrap();
    ...
    assert_eq!(
        hop1_out(&s2, "A"),
        1,
        "legacy u64-keyed edges.bin loads + traverses under u128"
    );
}
