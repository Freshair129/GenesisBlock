...
fn flush_clears_index_lag() {
    ...
    s.flush_index();
    assert_eq!(s.index_lag(), 0, "no vectors should remain unindexed after flush");
}
...
fn bulk_then_flush_indexes_all() {
    ...
    let inputs: Vec<NodeInput> = (0..30).map(|i| NodeInput {
        id: Some(format!("b{i}")), labels: vec![], props: None,
        embedding: Some(vec![i as f64, 1.0, 0.0]), lang: None, valid_from: None,
        caused_by: None, ttl: None, collection: None,
    }).collect();
