...
fn sq8_rerank_finds_exact_match() {
    ...
    assert!(
        info(&s, "c").rerank,
        "CollectionInfo reports rerank enabled"
    );
}
...
fn rerank_survives_reload() {
    let path = fresh("test_rr_reload");
    {
        ...
        let fvec = fs::metadata(Path::new(&path).join("fvec_c.bin"))
            .unwrap()
            .len();
        assert_eq!(
            fvec,
            2 * 4 * 4,
            "fvec sidecar = 2 vectors x dim 4 x 4 bytes"
        );
    }
...
fn no_rerank_writes_no_sidecar() {
    ...
    assert!(
        !Path::new(&path).join("fvec_c.bin").exists(),
        "no sidecar file for a non-rerank collection"
    );
}

/// A rerank collection whose `fvec_<name>.bin` is missing on reload (partial /
/// corrupt snapshot) recovers the exact sidecar from the canonical journal.
/// The durable collection definition and nearest-neighbor result both survive.
#[test]
fn missing_sidecar_recovers_from_journal() {
    ...
    assert!(
        info(&s, "c").rerank,
        "journal restores the configured sidecar"
    );
    assert_eq!(
        top1(&s, vec![1.0, 0.0, 0.0, 0.0], "c").as_deref(),
        Some("A"),
        "missing sidecar recovers the original search result from the journal"
    );
}
