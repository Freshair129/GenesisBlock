...
fn top1(s: &Storage, q: Vec<f64>, coll: &str, ef: Option<u32>) -> Option<String> {
    ...
    s.hybrid_search(HybridSearchInput {
        ...
        collection: Some(coll.to_string()), ef_search: ef,
...
fn ef_default_survives_reload() {
    ...
    assert_eq!(info(&s, "withef").ef_search, Some(77), "ef default persisted across reopen");
    assert_eq!(info(&s, "noef").ef_search, None, "absent ef_search loads as None");
}
...
fn collection_default_search_finds_exact() {
    ...
    assert_eq!(top1(&s, vec![0.0, 0.0, 1.0, 0.0], "withef", None).as_deref(), Some("C"));
}
