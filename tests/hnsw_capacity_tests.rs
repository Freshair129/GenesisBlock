...
fn many_collection_indexes_do_not_oom() {
    ...
    for i in 0..64 {
        let name = format!("c{i}");
        s.create_collection(name.clone(), "m".to_string(), dim as u32, None, None, None, None).unwrap();
        ...
        s.add_node(NodeInput {
            id: Some(format!("n{i}")), labels: vec![], props: None,
...
fn emb_for(i: u32, dim: usize) -> Vec<f64> {
    (0..dim).map(|j| (((i.wrapping_mul(2_654_435_761).wrapping_add(j as u32 * 40_503)) % 1000) as f64) / 1000.0).collect()
}
...
fn index_grows_past_initial_floor() {
    ...
    let nodes: Vec<NodeInput> = (0..n).map(|i| NodeInput {
        id: Some(format!("g{i}")), labels: vec![], props: None,
        embedding: Some(emb_for(i, dim)), lang: None, valid_from: None, caused_by: None, ttl: None,
    ...
    let count = s.list_collections().into_iter().find(|c| c.name == "default").unwrap().count;
