...
var PgvectorManager = class {
  ...
  async connect() {
    ...
    const sanitizedConn = config.connectionString ? config.connectionString.replace(/:[^:]+@/, ":****@") : `${config.user}@${config.host}:${config.port}/${config.database}`;
    ...
    try {
      ...
      if (extCheck.rows.length === 0) {
        ...
        try {
          ...
          throw new Error(`pgvector extension missing and could not be created: ${err.message}`);
        }
      }
      try {
  ...
  async insert(item) {
    ...
    const createdAt = item.metadata.created_at || (/* @__PURE__ */ new Date()).toISOString();
   ...
   */
  async search(vector, limit = 10, store) {
    let where = "";
