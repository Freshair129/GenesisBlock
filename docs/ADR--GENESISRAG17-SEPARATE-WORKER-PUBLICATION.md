...
3. The worker commits a graph-only candidate for Stage 13: source, parsed,
   ...
   vectors or derived summaries in this transaction. It flushes, checkpoints
   and reads back the native graph/SQLite state.
...
5. The worker keeps the enrichment result separate from the original decision,
   ...
   vectors for the candidate generation. It flushes/checkpoints indexes,
   performs native and worker lexical readback and runs the frozen benchmark
   before sending the Stage 15/16 write receipt.
...
7. Only a gate that allows publication permits the worker to write a prepared
   snapshot. The worker atomically replaces the publication pointer and its
   retained published-history membership, then sends
...
| Lexical | `ready` after actual indexed and queried rows | Worker-owned SQLite FTS5, `implementation: worker_sqlite_fts5` |
