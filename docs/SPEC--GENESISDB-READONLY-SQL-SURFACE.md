...
- **ไม่เห็น edges เลย** — วัดจากฐาน RAG จริง: engine โหลด 15,393 edges แต่
  `SELECT ... FROM edges` ตอบ `no such table` เพราะ projection เป็น node-only
  โดยโครงสร้าง (`grep "INSERT.*edges" src/lib.rs` ไม่มีผลลัพธ์) รายงานที่ join
  ตามความสัมพันธ์ — ซึ่งสำหรับ catalog แบบ SKU → Variant → Offer คือรายงาน
  เกือบทั้งหมด — **เขียนไม่ได้เลย** ไม่ใช่เขียนได้แต่ช้า
  เหตุผลตั้งต้นใน §2 ("ต้อง export ออกไปข้างนอก") จึงยังจริงอยู่ครึ่งหนึ่ง
  ดู `SPEC--GENESISDB-EDGE-PROJECTION` สำหรับข้อเสนอที่ยังรออนุมัติ
  (ซึ่งรวมทางเลือก "ไม่ทำ" ไว้อย่างจริงจัง)
