...
role: "Embedded↔embedded graph head-to-head: GenesisDB vs Kuzu"
phase: 26
...
depths {1,3,6}, 200 q/depth, LIMIT 1000. GenesisDB = P22 `graph-bench`. Both
embedded, C: SSD, fanout-8 random graph (independent draws, identical stats).

## 3. Results (100k nodes / 800k edges)

| Metric | GenesisDB | Kuzu | Winner |
|---|---|---|---|
| hop1 p50 | 22 µs | 3,653 µs | **GenesisDB ~166×** |
| hop3 p50 | 2.33 ms | 15.71 ms | **GenesisDB ~6.7×** |
| hop6 p50 | 4.40 ms | 60.91 ms | **GenesisDB ~14×** |
| Edge ingest | 24.4 s (durable WAL) | 0.4 s (COPY) | **Kuzu ~60×** |
| Memory (RSS Δ) | 1,057 MB | 97 MB | **Kuzu ~11×** |

(10k: GenesisDB hop1 23 µs vs Kuzu 2,424 µs; Kuzu ingest 0.1 s, RSS 88 MB.)

## 4. Reading — different sweet spots (honest)

- **GenesisDB wins point/local traversal latency** by 7–166×. Its adjacency-list
...
**Takeaway:** GenesisDB is the right tool for **low-latency agent memory**
...
Kuzu's 11× lower memory directly validates AUDIT--P22's finding: GenesisDB's
...
## 6. Program status
Completes the named-competitor matrix: **Chroma & Qdrant** (vector, P15/P20/P21),
**Neo4j** (server graph, P23), **Kuzu** (embedded graph, P26). Plus cost proofs
