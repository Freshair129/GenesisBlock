---
proposed_id: ADR--GENESISDB-SCALABILITY-VALIDATION
type: adr
status: candidate
aliases:
  - ADR
...
## Context
GenesisBlockDB claims a scalability target of **50 Million Nodes** and **500 Million Edges** within a single-machine embedded environment. For this to be defensible, we must provide a mathematical proof of memory consumption and an architectural strategy to overcome the limits of standard 64-bit pointers.
...
To avoid the 8-byte overhead of native 64-bit pointers, GenesisBlockDB uses **Internal Arena Indices** (u32).
...
## Consequences
*   **Positive:** Proves that GenesisBlockDB is "Systemically Fit" for large-scale cognitive graphs without requiring distributed clusters.
