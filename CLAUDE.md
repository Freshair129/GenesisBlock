...
- **HQL command families.** The current grammar/executor surface distinguishes five families: `SEARCH`, `TRAVERSE`, `MATCH (<pattern>)`, `MATCH <target> SIMILAR ...` (hybrid), and `CONTEXT`. Colon-bearing ids such as `user:5` must be quoted in command positions.
