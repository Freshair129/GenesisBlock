...
**Mobile / FFI surface (MARK XVI, v0.2.0):** the *same crate* also targets iOS/Android embedded — `--no-default-features --features mobile` drops napi, bins, and `sysinfo` (the RSS probe is bench-only; it'd fail in mobile sandboxes). `--features "mobile ffi"` adds `src/ffi.rs`, a `#[cfg(feature = "ffi")]` C ABI (8 `genesisdb_*` symbols over the sync `Storage` core) for the future iOS xcframework / Android JNI bridge. Desktop and mobile share one engine version — they are one source tree, different feature flags (see `docs/SPEC--MOBILE-SDK.md`). Cross-compile is validated by `.github/workflows/mobile-build.yml` (iOS/Android can't be built on the Windows dev host). Note: `sysinfo` is `optional` + `bins`-only, and `autobenches = false` keeps `cargo clippy --all-targets` from synthesizing phantom bench targets that ignore `required-features`.
