---
name: tiered-swarm
description: "Orchestrate a multi-step build/audit/refine task across local Ollama models + Codex under a token budget. Use when a job decomposes into many sub-tasks (verify, author, review), some cheap-and-machine-checkable and some hard, and you want to route each to the cheapest tier that can pass its acceptance check on local GPU hardware (e.g. an RTX 3060 12GB) before escalating to Codex. Spec-driven (composes spec-driven-development), TDD per task (composes tdd-workflow), cost-metered on every tier including local."
---
...
escalating up the ladder (local SLM → local mid → cloud open-weights → Codex) only when
...
- You have local Ollama models available and want to spend Codex tokens only where they
  earn their price.
...
3. **EXECUTE(tier)** — Dispatch the task to the hinted tier via Codex's built-in
   sub-agent (Task) mechanism for Codex tiers, or via the Ollama HTTP API for local /
...
4. **VERIFY (cheap gate)** — Run the `verify_command`. Tiered judge: deterministic check
   ...
   Codex judge only if still ambiguous. This gate is the economic interlock — no
...
| quality authoring / general | T2 Codex-mid | `Codex-sonnet-4-6` |
| hard reasoning / adversarial review gate | T3 Codex-frontier | `Codex-opus-4-8` |
...
- `scripts/cost_ledger.py` — the two-way cost formula over the four per-call counters
  ...
  local rate = 0 but metered. Re-verify Codex rates before any billing.

> NOTE: scripts in `scripts/` must NOT embed LLM API calls (`openai`, `anthropic`,
> `langchain`, etc.). Model invocation is the Agent/Task mechanism (for Codex) or the
...
- **`Codex-api`** — reference for current Codex model ids, per-type pricing, and prompt-
  caching mechanics. RE-VERIFY all Codex rates against
  `https://platform.Codex.com/docs/en/pricing.md` before any billing logic ships (the
