...
description: Check that a new engine capability is wired into BOTH front-ends — the NAPI method in src/lib.rs and the Axum REST route in src/router.rs — since AGENTS.md warns they drift. Use when adding or changing an engine operation, exposing a Storage method, reviewing whether the Node addon and REST server are in sync, or when the user says "wire this into both", "check NAPI/REST parity", "is this exposed over REST too".
---
...
`src/router.rs`. AGENTS.md warns: *"When you add an engine capability, you usually
...
- **NAPI-only** (intentional): e.g. `execute_batch` exists in the core but is
  deliberately **not** a REST route (per AGENTS.md). If you choose NAPI-only, say so
  and why.
...
```bash
bash .Codex/skills/napi-rest-parity/scripts/parity-list.sh
```
...
- **Body shape.** `/v1/query/hql` takes a **raw JSON string** body, but the
  ...
  changing either side (AGENTS.md gotcha). Check `tests/rest_api_tests.rs`.
