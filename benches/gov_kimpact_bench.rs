...
fn build_graph(storage: &Storage, n: usize, fanout: usize, rng: &mut StdRng) {
    ...
    for i in 0..n {
        for _ in 0..fanout {
            ...
            buf.push(EdgeInput { id: None, from: format!("g{i}"), to: format!("g{to}"), rel: "LINK".to_string(),
                props: None, valid_from: None, supersede: None, impact: None, caused_by: None });
            if buf.len() >= chunk { storage.bulk_add_edges(std::mem::take(&mut buf)).unwrap(); }
        }
    }
    if !buf.is_empty() { storage.bulk_add_edges(buf).unwrap(); }
}

fn open(bench: &str, sub: &str) -> Storage {
    let p = format!("{bench}/gk_{sub}");
    let _ = std::fs::remove_dir_all(&p);
    Storage::open(OpenOptions { path: p, page_cache_mb: Some(256), read_only: Some(false), vector_dim: Some(8) }).unwrap()
}
...
fn main() {
    ...
    let t = Instant::now();
    let mut acc = 0u64;
    for _ in 0..iters { acc = acc.wrapping_add(std::hint::black_box(labels_user.len() as u64)); }
    let base_ns = t.elapsed().as_nanos() as f64 / iters as f64;
    std::hint::black_box(acc);

    // guard ON: validate_governance (USER passes)
    let t = Instant::now();
    for _ in 0..iters { let _ = std::hint::black_box(s.validate_governance(std::hint::black_box(&labels_user), false)); }
    let guard_ns = t.elapsed().as_nanos() as f64 / iters as f64;

    // guard ON, MASTER (rejected path)
    let t = Instant::now();
    for _ in 0..iters { let _ = std::hint::black_box(s.validate_governance(std::hint::black_box(&labels_master), false)); }
    ...
    println!("  guard ON (USER)    {:.2} ns/op  -> overhead {:.2} ns/op", guard_ns, overhead);
    println!("  guard ON (MASTER)  {:.2} ns/op  (reject path)", guard_master_ns);
    println!("  context: a durable add_node is ~us-ms (WAL fsync); guard is ~{:.0}x cheaper",
        1000.0 / overhead.max(0.01));

    // ---------- P25: K-Impact full vs incremental ----------
    println!("\n== P25 K-Impact: full vs incremental recompute ==");
    println!("  {:>10} {:>16} {:>18} {:>14}", "nodes", "full (ms)", "incremental(1) us", "speedup");
    let sizes = std::env::var("GB_KIMPACT_SIZES").ok()
    ...
    for &n in &sizes {
        ...
        println!("  {:>10} {:>16.2} {:>18.3} {:>13.0}x", n, full_ms, inc_us, speedup);
    }
