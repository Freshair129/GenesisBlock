...
fn rss_mb() -> u64 {
    let mut s = sysinfo::System::new_all(); s.refresh_all();
...
fn main() {
    ...
    let depths: Vec<u32> = std::env::var("GB_GRAPH_DEPTHS").ok()
    ...
    let storage = Storage::open(OpenOptions {
        path: dbpath, page_cache_mb: Some(256), read_only: Some(false), vector_dim: Some(8),
    }).expect("open");

    println!("P22 graph: N={n} fanout={fanout} (edges≈{}) q={q}/depth limit={limit}", n * fanout);
    ...
    for i in 0..n {
        for _ in 0..fanout {
            ...
            buf.push(EdgeInput {
                id: None, from: format!("g{i}"), to: format!("g{to}"), rel: "LINK".to_string(),
                props: None, valid_from: None, supersede: None, impact: None, caused_by: None,
            });
            if buf.len() >= chunk { total_edges += buf.len(); storage.bulk_add_edges(std::mem::take(&mut buf)).unwrap(); }
        }
    }
    if !buf.is_empty() { total_edges += buf.len(); storage.bulk_add_edges(buf).unwrap(); }
    let edge_sec = t.elapsed().as_secs_f64();
    let rss = rss_mb();
    println!("ingest: {n} nodes {:.1}s, {total_edges} edges {:.1}s, RSS {rss} MB", node_sec, edge_sec);
    ...
    for &d in &depths {
        ...
        for _ in 0..q {
            ...
            let t0 = Instant::now();
            let res = storage.neighbors(seed, NeighborInput {
                depth: Some(d), rel: None, rels: None, direction: Some("out".to_string()),
                as_of: None, include_invalid: Some(false), limit: Some(limit),
            }, false).unwrap();
        ...
        println!("  hop{d}: p50 {:.1}µs p95 {:.1}µs p99 {:.1}µs | {:.0} trav/s | avg {} nodes",
            p50, p95, p99, tput, total_results / q);
